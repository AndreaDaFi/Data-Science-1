...
All scripts have been updated so that S3 download/upload calls are commented out. You can run every script directly on your machine as long as you have the dataset locally.
...
2. **Get the dataset**  
   ...
   To run preprocessing locally, place `dblp-ref-0.json` through `dblp-ref-3.json` in a `raw/` folder, then:
   ```
...
Run each script from the **root of the repo** (not from inside `src/`):
...
S3 uploads are skipped automatically — outputs stay local.

---

### Re-enabling the pipeline (S3 mode)

Each script has its S3 code preserved as comments, marked with `# --- PIPELINE (S3) ---` and `# --- LOCAL ---`. To restore a script for pipeline use:

1. Uncomment the `# --- PIPELINE (S3) ---` block
2. Comment out the `# --- LOCAL ---` block
3. Restore `import boto3` and the `BUCKET` constant at the top of the file
